...
class L2Adversary(object):
    ...
    def __init__(self, model,targeted=True, confidence=0.0, c_range=(1e-3, 1e10),
                 search_steps=1, max_steps=10, abort_early=True,
                 box=(-1., 1.), optimizer_lr=100, init_rand=False):
        """
        ...
        self.cos_loss = torch.nn.CosineSimilarity().cuda()
    ...
    def perturb(self,inputs,targets,to_numpy=True):
        """
        ...
        targets = make_cuda_consistent(self.model, targets)[0]  # type: torch.FloatTensor
        ...
        scale_consts_np = 100 * np.ones(batch_size)  #np.ones(batch_size) * self.c_range[0]
        ...
        inputs_tanh_var = Variable(inputs, requires_grad=False)

        # targets_tanh = self._to_tanh_space(targets)
        targets_tanh_var = Variable(targets, requires_grad=False)
        ...
        for sstep in range(self.binary_search_steps):
            ...
            for optim_step in range(self.max_steps):
                ...
                batch_loss, l2, cos_np, advxs_np = \
                    self._optimize(optimizer, self.model, inputs_tanh_var,
                                   pert_tanh_var, targets_tanh_var,
                                   scale_consts_var)
                # if optim_step % 10 == 0:
                ...
                for i in range(batch_size):
                    if (self.targeted and cos_np[i] < -0.6) or (not self.targeted and cos_np[i] < 0.25):
                        ...
                        if l2[i] < o_best_l2[i]:
                            ...
                            o_best_l2[i] = l2[i]
                            o_best_l2_ppred = 1
                            o_best_advx[i] = advxs_np[i]

            # binary search of `scale_const`
            # if best_l2_ppred != -1:
            #     # successful; attempt to lower `scale_const` by halving it
            #     if scale_consts_np < upper_bounds_np:
            #         upper_bounds_np = scale_consts_np
            ...
            #     if upper_bounds_np < self.c_range[1] * 0.1:
            #         scale_consts_np = (lower_bounds_np + upper_bounds_np) / 2
            # else:
            #     # failure; multiply `scale_const` by ten if no solution
            #     # found; otherwise do binary search
            #     if scale_consts_np > lower_bounds_np:
            #         lower_bounds_np = scale_consts_np
            #     if upper_bounds_np < self.c_range[1] * 0.1:
            #         scale_consts_np = (lower_bounds_np + upper_bounds_np) / 2
            #     else:
            #         scale_consts_np *= 10
        ...
        o_best_advx = self.where(o_best_advx > inputs_tanh_var + self.eps, inputs_tanh_var + self.eps, o_best_advx)
        o_best_advx = self.where(o_best_advx < inputs_tanh_var - self.eps, inputs_tanh_var - self.eps, o_best_advx)
        o_best_advx = torch.clamp(o_best_advx, -127.5, 127.5)
        # o_best_advx[:, 0, :, :] = torch.clamp(o_best_advx[:, 0, :, :], -self.mean_bgr[0], 255.0 - self.mean_bgr[0])
        # o_best_advx[:, 1, :, :] = torch.clamp(o_best_advx[:, 1, :, :], -self.mean_bgr[1], 255.0 - self.mean_bgr[1])
        # o_best_advx[:, 2, :, :] = torch.clamp(o_best_advx[:, 2, :, :], -self.mean_bgr[2], 255.0 - self.mean_bgr[2])
        # o_best_advx = o_best_advx.data.cpu().numpy()
        # if not to_numpy:
        #     o_best_advx = torch.from_numpy(o_best_advx).float()
        return o_best_advx

    def _optimize(self, optimizer, model, inputs_tanh_var, pert_tanh_var,
                  targets_tanh_var, c_var):
        """
        ...
        # targets_var = self._from_tanh_space(targets_tanh_var)

        advxs_var = inputs_tanh_var + pert_tanh_var  # type: Variable
        targets_var = targets_tanh_var
        # the perturbed activation before softmax
        pert_outputs_var = model(advxs_var)  # type: Variable
        targets_outputs_var = model(targets_var)
        # the original inputs
        inputs_var = inputs_tanh_var  # type: Variable

        #L8 constraints
        # perts_norm_var = torch.max(advxs_var - inputs_var)
        # print(perts_norm_var)
        # L2 constraints
        ...
        if self.targeted:
            ...
            f_var = - self.cos_loss(pert_outputs_var, targets_outputs_var)
        ...
        # pert_tanh_var = Variable(pert_tanh_var.data, requires_grad=True)
        # print(inputs_tanh_var)
        ...
        cos_np = _var2numpy(f_var)
        return batch_loss, pert_norms_np, cos_np, advxs_np
