...
class DefaultTrainer(TrainerBase):
    ...
    @classmethod
    def build_adv_test_loader(cls, cfg, dataset_name):
        return build_reid_adv_test_loader(cfg, dataset_name=dataset_name)
    ...
    @classmethod
    def build_adv_evaluator(cls, cfg, dataset_name, output_dir=None):
        data_loader, num_query = cls.build_adv_test_loader(cfg, dataset_name)
        return data_loader, ReidEvaluator(cfg, num_query, output_dir)
    ...
    def advtest(cls, cfg, model):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            logger.info("Prepare testing set")
            try:
                data_loader, evaluator = cls.build_adv_evaluator(cfg, dataset_name)
            except NotImplementedError:
                logger.warn(
                    "No evaluator found. implement its `build_evaluator` method."
                )
